...
class SVM():
	...
	def transform(self, X):
		"""Extends the original matrix with features and calculating kernel function for each element."""
	...
	def fit(self, X, y):
		"""
		Fitting SVM model using cvxopt library to solve quadratic optimization problem.
		...
		q = cvxopt.matrix(np.ones(num_observations) * -1)
		A = cvxopt.matrix(y, (1, num_observations))
		b = cvxopt.matrix(0.0)
		G = cvxopt.matrix(np.diag(np.ones(num_observations) * -1))
		h = cvxopt.matrix(np.zeros(num_observations))

		# Quadratic problem solver for calculating alphas
		alphas = np.ravel(cvxopt.solvers.qp(P, q, G, h, A, b)['x'])
		
		# Support vector have alphas that are not equeal to zero
		sv_flags = alphas > 1e-5

		# Storing values for support vectors
		self._support_vectors = X[sv_flags]
		self._alphas = alphas[sv_flags]
		self._support_labels = y[sv_flags]
		self._indices = np.arange(num_observations)[sv_flags]

		# Calculating bias term
		for i in range(self._alphas.shape[0]):
			self.b += self._support_labels[i] 
			self.b -= np.sum(self._alphas * self._support_labels * K[self._indices[i], sv_flags])
		self.b /= self._alphas.shape[0]

		if self.kernel == "linear":
			self.weights = np.sum(X * y.reshape(num_observations,1) * self._alphas.reshape(num_observations, 1), axis=0, keepdims=True)
